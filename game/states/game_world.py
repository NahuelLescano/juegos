...
class Player():
	...
	def animate(self, delta_time, direction_x, direction_y):
		...
		#if no direction pressed, set image to idle and return
		if not (direction_x or direction_y):
			self.curr_image = self.curr_anim_list[0]
			return

		#if an image was pressed, use the appropiate list of frames according the direction
		if direction_x:
			if direction_x > 0:
